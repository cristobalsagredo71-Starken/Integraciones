...
window.switchTab = (tabName) => {
    const tabs = ['pedidas', 'clientes', 'sistemas', 'conductor', 'metricas', 'kit'];
    tabs.forEach(t => {
...
function renderCharts() {
    try {
        ...
        const ctxOwner = document.getElementById('chartOwner');
        if (!ctxRev || !ctxVol || !ctxPhase || !ctxOwner) return;
        ...
        ctxPhase.parentElement.style.position = 'relative';
        ctxPhase.parentElement.style.height = '350px';
        ctxOwner.parentElement.style.position = 'relative';
        ctxOwner.parentElement.style.height = '350px';

        const sortedByRevenue = [...clientsData].filter(c => c.revenue > 0).sort((a,b) => b.revenue - a.revenue).slice(0, 10);
        const sortedByVolume = [...clientsData].filter(c => c.volume > 0).sort((a,b) => b.volume - a.volume).slice(0, 10);

        // Agrupar fases
        const phaseCounts = {};
        const ownerCounts = {};
        initiatives.forEach(i => {
            const p = i.phase || 'POR DEFINIR';
            const o = i.owner || 'POR DEFINIR';
        ...
        // Colors for Phase Donut
        const phaseColors = {
            'EVALUACION': '#f59e0b',
            'DISCOVERY': '#3b82f6',
            'DISENO': '#8b5cf6',
            'IMPLEMENTACION': '#ec4899',
            'QA': '#14b8a6',
            'GO_LIVE': '#10b981',
        ...
        const phaseData = Object.values(phaseCounts);
        const phaseBg = phaseLabels.map(p => phaseColors[p] || '#6b7280');

        if (window.chartPhaseInstance) window.chartPhaseInstance.destroy();
        window.chartPhaseInstance = new Chart(ctxPhase, {
            type: 'doughnut',
            data: {
                labels: phaseLabels,
                datasets: [{
        ...
        const ownerData = Object.values(ownerCounts);
        const ownerBg = ownerLabels.map(o => ownerColors[o] || '#6b7280');

        if (window.chartOwnerInstance) window.chartOwnerInstance.destroy();
        window.chartOwnerInstance = new Chart(ctxOwner, {
            type: 'doughnut',
            data: {
                labels: ownerLabels,
                datasets: [{
        ...
        if (window.chartRevenueInstance) window.chartRevenueInstance.destroy();
        window.chartRevenueInstance = new Chart(ctxRev, {
            type: 'bar',
            data: {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                scales: {
                    x: { grid: { color: gridColor }, ticks: { callback: (val) => '$' + (val/1000000).toFixed(1) + 'M' } },
        ...
        if (window.chartVolumeInstance) window.chartVolumeInstance.destroy();
        window.chartVolumeInstance = new Chart(ctxVol, {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                scales: {
                    x: { grid: { color: gridColor }, ticks: { callback: (val) => (val/1000).toFixed(0) + 'k' } },
                    y: { grid: { display: false } }
                },
                plugins: {
...
// Subir archivos de Kit de Onboarding
window.uploadKitFile = async (inputId, fileName) => {
    const fileInput = document.getElementById(inputId);
    const statusDiv = document.getElementById('kit-upload-status');
    if (!fileInput.files || fileInput.files.length === 0) {
        statusDiv.style.color = 'var(--warning)';
        statusDiv.textContent = 'Selecciona un archivo primero.';
        return;
    }
